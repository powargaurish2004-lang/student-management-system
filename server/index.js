...
app.post("/api/auth/register", async (req, res) => {
  try {
    ...
    if (!["user", "admin"].includes(role)) return res.status(400).json({ message: "Choose a valid account type." });
    if (role === "admin") {
...
app.post("/api/auth/login", async (req, res) => {
  ...
  if (req.body.role && req.body.role !== role) return res.status(403).json({ message: `This account is registered as a ${role}.` });
  if (role === "admin" && user.email !== adminEmail) return res.status(403).json({ message: "This admin account is not authorized." });
  const token = jwt.sign({ userId: user.id, role }, jwtSecret, { expiresIn: "7d" });
...
app.put("/api/students/:id", auth, async (req, res) => {
  if (req.role !== "admin") return res.status(403).json({ message: "Only admins can edit students." });
  const message = validateStudent(req.body); if (message) return res.status(400).json({ message });
...
app.delete("/api/students/:id", auth, async (req, res) => {
  ...
  const deleted = await Student.findOneAndDelete({ _id: req.params.id }); if (!deleted) return res.status(404).json({ message: "Student not found." }); res.status(204).end();
});
